...
equation_loss = BurgersEqnLoss(
    method=config.opt.get("pino_method", "fdm"), visc=0, loss=F.mse_loss
)
...
# -----------------------------
torch.save(model.state_dict(), "pino_inviscid_burgers_last_model_eqn_1_ic_10.pt")
print("Saved final model to pino_inviscid_burgers_last_model_eqn_1_ic_10.pt")
...
with open("pino_inviscid_burgers_losses_eqn_1_ic_10.json", "w") as f:
    json.dump(loss_data_clean, f, indent=2)

print("Saved train/test losses → pino_inviscid_burgers_losses_eqn_1_ic_10.json")
...
plt.tight_layout()
plt.savefig("loss_curve_inviscid_eqn_1_ic_10.png", dpi=300)
plt.close()


print("Saved loss curve → loss_curve_inviscid_eqn_1_ic_10.png")
...
plt.grid(True, which="both")  # <-- optional: show grid for log ticks
plt.tight_layout()
plt.savefig("loss_curve_equation_ic_inviscid_eqn_1_ic_10.png", dpi=300)
plt.close()
