...
for epoch in range(config.opt.n_epochs):
    ...
    # If using an adaptive loss aggregator, save the weights too
    if hasattr(train_loss, "weights"):
        for i, name in enumerate(config.opt.training_loss):
            loss_weights_history[name].append(weights[i])
...
# -----------------------------
torch.save(model.state_dict(), "pino_burgers_v_0001_last_model_res_100_modified_ite_10000.pt")
print("Saved final model to pino_burgers_v_0001_last_model_res_100_modified_ite_10000.pt")
...
loss_data = {
    ...
    "train_loss_weights": loss_weights_history,      # if you're storing them
}

loss_data_clean = to_serializable(loss_data)

with open("pino_burgers_v_0001_losses_res_100_modified_ite_10000.json", "w") as f:
    json.dump(loss_data_clean, f, indent=2)

print("Saved train/test losses → pino_burgers_v_0001_losses_res_100_modified_ite_10000.json")
...
plt.tight_layout()
plt.savefig("loss_curve_burgers_v_0001_res_100_modified_ite_10000.png", dpi=300)
plt.close()


print("Saved loss curve → loss_curve_burgers_v_0001_res_100_modified_ite_10000.png")
...
plt.grid(True, which="both")  # <-- optional: show grid for log ticks
plt.tight_layout()
plt.savefig("loss_curve_equation_ic_burgers_v_0001_res_100_modified_ite_10000.png", dpi=300)
plt.close()
