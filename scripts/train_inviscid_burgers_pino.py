...
# -----------------------------
torch.save(model.state_dict(), "pino_inviscid_burgers_last_model.pt")
print("Saved final model to pino_inviscid_burgers_last_model.pt")
...
with open("pino_inviscid_burgers_losses.json", "w") as f:
    json.dump(loss_data_clean, f, indent=2)

print("Saved train/test losses → pino_inviscid_burgers_losses.json")
...
plt.tight_layout()
plt.savefig("loss_curve_inviscid.png", dpi=300)
plt.close()


print("Saved loss curve → loss_curve_inviscid.png")
...
plt.grid(True, which="both")  # <-- optional: show grid for log ticks
plt.tight_layout()
plt.savefig("loss_curve_equation_ic_inviscid.png", dpi=300)
plt.close()
