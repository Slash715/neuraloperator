...
from neuralop import H1Loss, LpLoss, BurgersEqnLoss, HeatBurgersEqnLoss, ICLoss, get_model
...
h1_loss = H1Loss(d=2)
ic_loss = ICLoss()
equation_loss = HeatBurgersEqnLoss(
    method=config.opt.get("pino_method", "fdm"), visc=0.1, loss=F.mse_loss
)
...
# Manual weights (PDE, IC)
w_equation = 1.0
w_ic = 10.0

print("\n### LOSSES ###")
print("\n * Train: Manual weighted sum (equation=1.0, ic=10.0)")
print(f"\n * Test: {eval_losses}")
...
# NEW: per-loss history (equation, ic, etc.)
per_loss_history = {name: [] for name in config.opt.training_loss}

# OPTIONAL: track adaptive weights for each loss if using Relobralo / SoftAdapt
#loss_weights_history = {name: [] for name in config.opt.training_loss}
...
for epoch in range(config.opt.n_epochs):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        # Manual weighted sum
        total_loss = w_equation * loss_vals["equation"] + w_ic * loss_vals["ic"]
        weights = [w_equation, w_ic]  # for printing/logging only
    ...
    # Save per-loss averages for this epoch
    for name in config.opt.training_loss:
        per_loss_history[name].append(avg_losses[name])
...
# -----------------------------
torch.save(model.state_dict(), "pino_heat_burgers_v_01_last_model_eqn_1_ic_10_res_100_modified_ite_10000.pt")
print("Saved final model to pino_heat_burgers_v_01_last_model_eqn_1_ic_10_res_100_modified_ite_10000.pt")
...
def to_serializable(obj):
    if isinstance(obj, torch.Tensor):
        # scalar tensor → float, multi-dim → nested list
...
loss_data = {
    "epochs": all_epochs,
    "train_total_losses": all_train_losses,
    "train_component_losses": per_loss_history,      # 'equation', 'ic', etc.
    "test_losses": all_test_losses,
}

loss_data_clean = to_serializable(loss_data)

with open("pino_heat_burgers_v_01_losses_eqn_1_ic_10_res_100_modified_ite_10000.json", "w") as f:
    json.dump(loss_data_clean, f, indent=2)

print("Saved train/test losses → pino_heat_burgers_v_01_losses_eqn_1_ic_10_res_100_modified_ite_10000.json")

# -----------------------------
# Plot and save PNG curve
# -----------------------------
plt.figure(figsize=(8, 6))

# Total training loss (Relobralo-aggregated)
plt.plot(all_epochs, all_train_losses, label="Total Training Loss", linewidth=2)

# Per-component losses (equation, ic, etc.)
for name in config.opt.training_loss:
    plt.plot(all_epochs, per_loss_history[name],
             label=f"{name.upper()} Loss", linewidth=1.5)

# Test loss
if len(all_test_losses) > 0:
    test_epochs = list(
        range(0, len(all_test_losses) * config.opt.eval_interval, config.opt.eval_interval)
    )
    plt.plot(test_epochs, all_test_losses, label="Test L2 Loss", linewidth=2)

plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training & Test Loss Curves (PINO Burgers)")
plt.yscale("log")          # <-- add this
plt.legend()
plt.grid(True, which="both")  # <-- optional: show grid for log ticks
plt.tight_layout()
plt.savefig("loss_curve_heat_viscous_v_01_eqn_1_ic_10_res_100_modified_ite_10000.png", dpi=300)
plt.close()


print("Saved loss curve → loss_curve_heat_viscous_v_01_eqn_1_ic_10_res_100_modified_ite_10000.png")

plt.figure(figsize=(8, 6))
plt.plot(all_epochs, per_loss_history["equation"], label="EQUATION Loss", linewidth=2)
plt.plot(all_epochs, per_loss_history["ic"], label="IC Loss", linewidth=2)
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("PINO Burgers: EQUATION vs IC Loss")
plt.yscale("log")          # <-- add this
plt.legend()
plt.grid(True, which="both")  # <-- optional: show grid for log ticks
plt.tight_layout()
plt.savefig("loss_curve_equation_ic_heat_viscous_v_01_eqn_1_ic_10_res_100_modified_ite_10000.png", dpi=300)
plt.close()
